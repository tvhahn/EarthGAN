...
class EarthDataTrain(Dataset):
    def __init__(
        ...
        self.time_step_list = list(
            set(re.findall("[0-9]+", str(i))[-1] for i in file_list_truth)
        )
        ...
        assert (
            len(check_1) == 0
        ), f"missing truth or input time step sample(s): {check_1}"
        assert (
            len(check_2) == 0
    ...
    def __getitem__(self, index):
        ...
        up_data = F.interpolate(
            input_data, scale_factor=(1, 8, 8), mode="trilinear", align_corners=False
        )
