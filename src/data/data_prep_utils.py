...
def load_data(data_path: pathlib.PosixPath):
    ...
    return (
        np.expand_dims(np.moveaxis(x, source=[3, 0], destination=[0, 2]), 0),
        var_list,
        radial_array,
    )
...
def create_truth_and_downsampled_x(
    file, downscale_percent_1=0.60, downscale_percent_2=1 / 8.0
):
...
def scaler(x, min_val_array, max_val_array):
    ...
    for radius in range(r):
        for var in range(v):
            x[0, var, radius, :, :] = np.divide(
                (x[0, var, radius, :, :] - min_val_array[var]),
                np.abs(max_val_array[var] - min_val_array[var]),
            )
