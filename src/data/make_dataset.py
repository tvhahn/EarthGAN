...
import numpy as np
import re
from src.data.data_prep_utils import create_truth_and_downsampled_x, min_max_array, scaler
...
def main(input_filepath, interim_filepath, output_filepath):
    ...
    folder_raw_data = root_dir / input_filepath # raw data folder that holds the .nc files


    folder_input_data_interim = root_dir / interim_filepath / 'input' # 8x downsampled data - unscaled
    folder_truth_data_interim = root_dir / interim_filepath/ 'truth' # "original" data (60% of native file h & w) - unscaled
    ...
    for i, file in enumerate(file_list):
        ...
        x = scaler(np.load(folder_input_data_interim / x_save_name), min_array_input, max_array_input)
        np.save(folder_input_data / x_save_name, x)

        x_truth = scaler(np.load(folder_truth_data_interim / x_truth_save_name), min_array_truth, max_array_truth)
